...
export default class UploadImages extends Component {
  ...
  uploadImages() {
    ...
    if (this.state.position_state.length != 0) {
      ...
      let cont = []
      console.log(this.state.vettoreFotoOld)
                ...
                if (cont.length == this.state.selectedFiles.length + this.state.vettoreFotoOld.length + conteggio) {
                  ...
                  uploadFoto(this.props.id, this.props.indice, cont, (res, all_foto) => {
                    if (res == "UploadCompleto") {
                      this.setState({ upload_completo: true, caricamento: false, selectedFiles: undefined }, () => {
                        console.log("ok")
                        ...
                        this.setState({ vettoreFotoAfterUpload: appoggioAfterUpload })
                      })
                    }
                  })
                }
  ...
  handleChange = ({ fileList }) => {
    ...
    if (fileList.length >= 1) {
      ...
      this.setState({ vettoreFotoOld: appoggio, cancellazione: true })
      /*
      Swal.fire({
        icon: 'error',
        title: 'Errore!',
